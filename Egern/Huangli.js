...
/**
 * ==========================================
 * 📌 代码名称: 📅 日历 / 老黄历 (自动星座推算 + 可在变量环境修改 + 第几周)
 * ==========================================
 */
export default async function(ctx) {
  ...
  const Lunar = {
    ...
    parse(y, m, d) {
      ...
      for (i = 1900; i < 2101 && offset > 0; i++) {
        ...
        temp += (this.info[i - 1900] & 0xf)
          ? ((this.info[i - 1900] & 0x10000) ? 30 : 29)
          : 0;
        offset -= temp;
      }
      ...
      const lYear = i;
      const leap = this.info[lYear - 1900] & 0xf;
      let isLeap = false;
      ...
      const cnDay =
        ...
        lD === 20 ? "二十" :
        lD === 30 ? "三十" :
        ["初","十","廿","卅"][Math.floor(lD / 10)] +
        ["日","一","二","三","四","五","六","七","八","九","十"][lD % 10];
  ...
  const shichenStr =
    ["子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥"][Math.floor((now.getHours() + 1) % 24 / 2)] + "时";
  let apiData = {};
  ...
  const getVal = (...keys) => {
    for (let k of keys) if (apiData[k]) return apiData[k];
    return "";
  };
  ...
  if (!chongshaInfo || chongshaInfo === "无") {
    const cycle =
      (Math.round((Date.UTC(Y, M - 1, D) - Date.UTC(1900, 0, 31)) / 86400000) + 40) % 60;
    const dCycle = cycle < 0 ? cycle + 60 : cycle;
    const dZhi = dCycle % 12;
    const cIndex = (dCycle + 6) % 60;
    chongshaInfo = `冲${
      "鼠牛虎兔龙蛇马羊猴鸡狗猪"[(dZhi + 6) % 12]
    }(${"甲乙丙丁戊己庚辛壬癸"[cIndex % 10]}${"子丑寅卯辰巳午未申酉戌亥"[cIndex % 12]})煞${["南","东","北","西"][dZhi % 4]}`;
  }
  ...
  for (let i = 1; i <= 365; i++) {
    const tempDate = new Date(todayMs + i * 86400000);
    const ty = tempDate.getFullYear(), tm = tempDate.getMonth() + 1, td = tempDate.getDate();
    const tl = Lunar.parse(ty, tm, td);
    ...
    children: [
...

      {
        ...
        children: [
          {
            type: "text",
            text: `${obj.gz}(${obj.ani})年 ${obj.cn} ${shichenStr}${obj.term ? ` · ${obj.term}` : ``}`,
...

          {
            ...
            children: [
              ...
              { type: "text", text: "节假日:", font: { size: 12, weight: "bold" }, textColor: C.holiday },
              {
                type: "text",
                text: finalHolidayText,
                font: { size: 12, weight: "medium" },
                textColor: C.sub,
                maxLines: 1,
                flex: 1
